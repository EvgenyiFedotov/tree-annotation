...
const getData = (params = {}) => {
  ...
  const dataByBlocks = getDataByBlocks(nodeFile.program.body);

  return [
    { type: "header", params: { value: "Main header", level: 1 } },
    ...dataByBlocks.types,
    ...dataByBlocks.interfaces,
    ...dataByBlocks.variables,
    ...dataByBlocks.functions,
    ...dataByBlocks.classes,
    ...dataByBlocks.exports
  ];
};

const getDataByBlocks = (nodes = []) => {
  const nodesByTypes = mdNodes.getNodesByTypes(nodes);

  const block = (func, nodes) => (nodes && nodes.length ? func(nodes) : []);

  return {
    types: block(getTypesData, nodesByTypes.TSTypeAliasDeclaration),
    interfaces: block(getInterfacesData, nodesByTypes.TSInterfaceDeclaration),
    variables: block(getVariablesData, nodesByTypes.VariableDeclaration),
    functions: block(getFunctionsData, nodesByTypes.FunctionDeclaration),
    classes: block(getClassesData, nodesByTypes.ClassDeclaration),
    exports: block(getExportsData, nodesByTypes.ExportNamedDeclaration)
  };
};
    ...
    node.declarations.forEach(nodeDeclaration => {
      memo.push([
        ...
        mdAnnotation.annotationNode(nodeDeclaration.id.typeAnnotation) ||
          mdAnnotation.annotationNode(nodeDeclaration.init) ||
          "unknow"
      ]);
    });
    return memo;
...
const getFunctionData = node => {
  ...
  const argsRows = node.params.map((nodeParam, index) => {
    ...
    switch (nodeParam.type) {
      ...
      case "ObjectPattern":
        name = `arg${index}`;
        annotation = mdAnnotation.annotationNode(nodeParam);
        required = "true";
        break;
    }
...
const getExportsData = (nodes = []) => {
  const nodesDeclarations = nodes.map(node => node.declaration);
  const dataByBlocks = getDataByBlocks(nodesDeclarations);

  const header = { type: "header", params: { value: "Exports" } };
