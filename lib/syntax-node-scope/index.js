...
const functionParse = node => {
  ...
  const params = fuctionParamsParse(node.params);
  const returns = nodeAnnotation(node.returnType);
  const returnsBody = functionReturnsBody(node.body);

  return { id, generator, async, params, returns, returnsBody };
};

const fuctionParamsParse = (nodes = []) => {
  const annotations = nodes.map(nodeAnnotation);
